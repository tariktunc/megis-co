// Tool.tsx — ciceksepeti-komisyon-hesaplama interactive component.
...
interface CiceksepetiToolState {
  purchasePrice: number;
  salePrice: number;
  vatRatePercent: number;
  /** User-entered, read from the seller's own Çiçeksepeti panel — never a WebForge-asserted rate (Rule 4). */
...
export function CiceksepetiTool() {
  const [state, update] = useToolState<CiceksepetiToolState>({
    purchasePrice: 0,
    salePrice: 0,
    vatRatePercent: 20,
  ...
  return (
    <div className="rounded-[1px] border border-border p-4 sm:p-6">
      <p className="mb-4 text-sm text-muted-foreground">
        Çiçeksepeti komisyon oranları kategoriye ve satıcı anlaşmanıza göre değişir.
        Çiçeksepeti bu oranları herkese açık tek bir tablo halinde yayınlamaz | bu
      ...
      <form className="flex flex-col gap-2" onSubmit={(e) => e.preventDefault()}>
        ...
        <NumberField
          ...
          min={0}
          max={100}
          helperText="Satıcı panelinizdeki Çiçeksepeti komisyon oranını girin."
          required
        />
