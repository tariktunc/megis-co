// page.tsx — amazon-komisyon-hesaplama page (Server Component).
...
export async function generateMetadata({
  ...
  return {
    ...
    description:
      locale === 'en'
        ? undefined
        : "Amazon'da FBA ücreti ve komisyonu kendi verilerinizle düşerek net kârı ve marjı hesaplayıp kanalın değip değmediğini görün.",
    alternates: {
      canonical: `${SITE_URL}${locale === 'en' ? '/en' : ''}/hesaplama-araclari/amazon-komisyon-hesaplama`,
    },
...
export default async function AmazonPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
      ...
      formula={
        <div className="text-foreground">
          ...
          <p className="mt-2 font-mono text-sm sm:text-base">
            Net Ödeme = Satış Fiyatı - Komisyon Tutarı - Kargo Ücreti (satıcı ödüyorsa) - FBA Ücreti (varsa)
          </p>
