// page.tsx — ciceksepeti-komisyon-hesaplama page (Server Component).
...
import { CiceksepetiTool } from './Tool';
...
export async function generateMetadata({
  ...
  return {
    ...
    description:
      locale === 'en'
        ? undefined
        : "Çiçeksepeti'nde komisyon, kargo ve alış maliyeti düşüldükten sonra elinizde kalan net kârı, kendi verilerinizle hesaplayın.",
    alternates: {
...
export default async function CiceksepetiPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  ...
  return (
    <ToolShell
      slug="ciceksepeti-komisyon-hesaplama"
      title="Çiçeksepeti Komisyon Hesaplama"
      answerBlock={guardText(answerBlock)}
      breadcrumbItems={[
        { label: 'Ana Sayfa', href: '/' },
        { label: 'Hesaplama Araçları', href: '/hesaplama-araclari' },
        { label: 'Çiçeksepeti Komisyon Hesaplama' },
      ]}
      toolSlot={<CiceksepetiTool />}
